...
"""Push an assetgen stage to Kaggle as a headless GPU script kernel, stream ntfy progress, download outputs.
    ...
    python kaggle_run.py concepts [--env KEY=VAL ...] [--sources everaldtah/nd-assetgen-concepts]
    python kaggle_run.py trellis  --sources everaldtah/nd-assetgen-concepts
"""
...
def main():
    ...
    (HERE / "build" / f"{a.stage}.topic").write_text(topic)
    if not a.no_wait:
        watch(a.stage, topic)
